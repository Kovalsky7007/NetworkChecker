...
NetworkChecker\
  ...
  NetworkChecker.ps1            ← единственный файл скрипта
...
> Это **внутренний** `$status`. На экран с v1.14.6 выводится **вердикт простым
> языком** (`Get-Verdict`), который отдельно учитывает DPI/RST, TLS-таймаут,
> заглушку (`TRUSTED*` + HTTP-фейл), `MITM` и «нет DNS-записи». Подробнее —
> раздел про колонку вывода ниже.
...
"  vk.com/  "                      → "vk.com"
"//t.me"                           → "t.me"
"://google.com"                    → "google.com"   (v1.14.3)
":" / "-" / "localhost"            → ""             (отбраковка, v1.14.3)
```

**Ужесточение (v1.14.3):** добавлены снятие схемы `*://`, ведущих `:`/`/`, порта
`:NNNN` и финальная валидация — результат должен содержать точку и только
`[a-zA-Z0-9.-]`. Иначе битые строки списков (`://apple.com` давал `:`) уходили в
DNS-резолв как мусор. Теперь такие строки возвращают `""` и отбрасываются
фильтром `Where-Object { $_ }`.
...
**Архитектура (v1.14.2/.3):** зависит от `$global:ShowLegend` (переключается кнопкой `L`):
- **ON** — `Write-TwoColumns` с двумя колонками: левая (LeftW=36) — логотип + IP/Geo +
  пункты меню; правая — `$global:LegendData[$lang]`. Итого 36 + 42 = 78 столбцов
  (раньше было 38+42/43 = 80–81 → перенос строк на консоли 80, интерфейс «плыл»).
- **OFF** — одноколоночное меню во всю ширину, IP/Geo не обрезаются.
...
### 6.2 `$global:LegendData` и `Format-Box`

**Назначение:** Единый источник данных для легенды правой колонки меню.

**Сборка (v1.14.2):** строится функцией `Format-Box -Title .. -Inner 40 -Rows ..`,
которая программно дополняет каждую строку до фиксированной ширины и добавляет
рамку `┌│└─`. Раньше рамки рисовались вручную и расходились на 1–2 символа
(«плывущий» UI) — теперь выравнивание гарантировано арифметикой, а не подсчётом
пробелов. `Format-Box` используется и для браузеров `lists\`/`Certs\`.

**Структура:** Хэш с ключами `"RU"` и `"EN"`, каждый = массив pscustomobject `{T; C}`.

**Кнопка `L`:** в `switch` меню (`"L"`/`"Л"`) переключает `$global:ShowLegend`
(показать/скрыть легенду). Отдельной функции-overlay `Show-Legend` больше нет —
убрана в v1.14.2 (был дубль определения), её роль выполняет правая колонка меню.
...
### 6.5 `Show-SideHelp` — удалена

Функция контекстных подсказок не вызывалась с v1.14.1 и **полностью удалена в
v1.14.2** (~170 строк мёртвого кода с криво выровненными рамками).

### 6.6 `Show-NetMonitor`

**Назначение:** Сетевой монитор в реальном времени. Переписан в v1.14.3.

**Анти-мерцание:** один `Clear-Host` при входе, далее каждый кадр рисуется через
`[Console]::SetCursorPosition(0,0)` с дополнением строк до ширины окна и затиранием
хвоста предыдущего кадра. Раньше `Clear-Host` на каждом тике дёргал курсор и мигал.
Тело таблицы обрезается по `[Console]::WindowHeight`, чтобы длинный список не
вызывал прокрутку (лишнее показано как «… ещё N»).

**Источник данных:** `netstat -ano` — парсим вывод регулярками. Базовый набор: TCP
ESTABLISHED/SYN/LISTEN + UDP :53.

**Интерактив (опрос `[Console]::KeyAvailable` во время тика):**
- `Q`/`Й` — выход
- `S`/`Ы` — сортировка: Состояние → Процесс → Гео → Адрес
- `F`/`А` — фильтр/исключение: Все → Активные → Без LISTEN → Только внешние
  (последний отбрасывает loopback, `10.*`, `192.168.*`, `172.16–31.*`, `*::1*`, `fe80*`)
- `P`/`З` — пауза автообновления (использует последний снимок `$rows`)

**Цвета строк:** `ESTABLISHED`→зелёный, `SYN_*`→красный, `LISTENING`→голубой,
`UDP:53`→тёмно-жёлтый.

**Geo кэш:** `$global:GeoCache` — заполняется по мере появления новых IP, сбрасывается
в `finally` при выходе (там же восстанавливается видимость курсора).
...
**Колонка вывода (v1.14.6+):** первая колонка таблицы — не сырой `UP/DEGRADED`,
а **вердикт простым языком** из `Get-Verdict` (`Доступен` / `Без страницы` /
`Блок DPI/RST` / `Блок TLS` / `Заглушка` / `MITM-перехват` / `Нет DNS-записи`).
Внутренний `$status` всё ещё считается в `Test-Domain`, но на экран идёт вердикт.
Цвет строки — по классу вердикта (`Get-VerdictColor`): OK→зелёный,
MINOR→DarkCyan, BLOCK→красный, DOWN→серый.

**Summary строка (по классам):**
```
Итог: 37   Доступно:24  Без стр.:8  Блок:4  Нет DNS:1   Avg Ping=33ms
```
Считается из объектов результата по полю `VClass` (`Write-ResultSummary`), а не
regex'ом по тексту строк.

**Avg Ping в Summary:** среднее по числовым значениям пинга (timeout/N/A не
учитываются).
...
```powershell
powershell -ExecutionPolicy Bypass -File NetworkChecker.ps1 -DebugMode
```
...
**Локализация (v1.14.4+):** `Check-Single` (запрос пингов, вывод, MITM-предупреждение)
и подсказки выбора списков переведены через ключи `$global:T` (RU/EN). Колонка
вердикта и DNS-проверка — двуязычны.

**Не локализовано (намеренно):**
- `Save-Log` — шапка отчёта с русскими подписями полей (лог идёт в техподдержку)
- `Select-CertFile`, `Manage-Schedule` — часть диалогов на русском

---

## 12. Цветовая логика

Цвет строки таблицы — по **классу вердикта** (`Get-VerdictColor`):

| Класс вердикта | Цвет | PowerShell |
|----------------|------|-----------|
| OK (`Доступен`) | Зелёный | `Green` |
| MINOR (`Без страницы`) | Тёмно-голубой | `DarkCyan` |
| BLOCK (`Блок…`/`Заглушка`/`MITM`) | Красный | `Red` |
| DOWN (`Нет DNS-записи`) | Серый | `DarkGray` |
...
**7. UI:**
```
$global:T + T() → Format-Box → $global:LegendData → Write-TwoColumns →
Show-IPInfo → Show-Manual → Show-NetMonitor
```
...
*Документ описывает NetworkChecker v1.14.3. Все технические решения задокументированы в комментариях к коду. Для воспроизведения читай комментарии вместе с этим документом — они дополняют друг друга. Список изменений — в [CHANGELOG.md](CHANGELOG.md).*
